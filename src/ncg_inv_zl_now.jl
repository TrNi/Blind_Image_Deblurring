"""
(x, out) = ncg_inv_zl(B, gf, reg, x0; [options])
...
where each `f_j` is a non-weighted quadratic function.
...
function ncg_inv_zl_now(
        ...
        for iter = 1:niter
                ...
                alf_sol = - sum([(dir' * (B[j]' * (gf[j](Bx[j])))) for j = 1:J]) /
                                sum([(reg[j] * (Bd[j]' * (Bd[j]))) for j = 1:J])
